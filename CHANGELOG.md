...
- Sessions that end while Sortie is under load now keep an accurate usage record: the model name and request count are no longer missing or too low, and tokens spent during self-review are no longer undercounted. A session that starts on an issue right after the previous one ended no longer inherits that session's token usage or self-review progress, which counted the same tokens twice against `agent.max_tokens` and could stop work on the issue before its budget was spent.
