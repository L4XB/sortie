...
- The `kiro`, `copilot-cli`, and `opencode` agent kinds, workspace hooks, the reaction triage command, and the self-review phase no longer wait indefinitely, or misreport a successful exit as a failure, when a surviving descendant process holds their output open. The handoff-evidence check no longer waits indefinitely for such a descendant, and no longer misreports a successful exit as a failure when the descendant stays inside the command's process tree; one that detaches into its own session still leaves the check unable to finish within its wait, and it reports the inspection as failed. On Windows, a process the `claude-code`, `copilot-cli`, `kiro`, `opencode`, `codex`, or `agent-client-protocol` runtime starts no longer escapes termination with its session or turn. A launch whose Job Object cannot be created still runs, as it did before, the failure is logged, and only its direct process is reached when it ends.
  ([#1080](https://github.com/sortie-ai/sortie/issues/1080))

### Changed

- On Linux and macOS, a process that a workspace hook, the reaction triage command, or a self-review verification command leaves running in its process group is now terminated when the command exits, matching what Windows hooks already did; a verification command's leftover processes on Windows are now terminated too. A service meant to outlive the command should now start through one of the routes the workflow reference lists for its platform, such as a service manager; hooks and the reaction triage command now receive `XDG_RUNTIME_DIR` and `DBUS_SESSION_BUS_ADDRESS` so they can start a systemd user service. A hook, the reaction triage command, or a self-review verification command that exits on its own and leaves such a process now logs one new INFO record, `leftover processes terminated after the command exited`. A launch's process-group termination, or its Job Object termination on Windows, now resends on a fixed poll interval until the group or job reports no member left or a 2-second bound elapses, on every reap rather than only inside a capture's own drain; a launch that still cannot prove the tree torn down once that bound elapses logs one new WARN record, `subprocess group termination failed after the launch returned`, carrying `command` and `error`. On Windows, the warning `hook process tree did not settle` is renamed `subprocess tree did not settle` and now logs for every launch that captures output, without `timeout_ms`, `wait_delay_expired`, or `resumed_threads` and with a new `drain_terminate_err` field when a termination of the process tree failed; the warnings `hook job object creation failed; child tree may survive timeout` and `hook process resume failed` are renamed `process group assignment failed` and `process resume failed` and now log for every Windows launch with `command`, `dir`, and `error` in place of `dir`, `timeout_ms`, and `error`; and the warnings `hook job termination after wait failed; drain may not settle`, `hook job accounting query failed; drain skipped`, and `hook job processes still active after drain deadline` are no longer logged, their content now carried by `subprocess tree did not settle` in the fields `drain_terminate_err`, `drain_query_err`, and `active_last`.
  ([#1080](https://github.com/sortie-ai/sortie/issues/1080))
