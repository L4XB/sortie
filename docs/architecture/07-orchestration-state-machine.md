...
- `Agent Update Event`
  ...
  - Every agent update and self-review progress message a worker delivered before its exit is applied before that exit is handled, so no message from one run is applied after its exit or to a later run of the same issue. A usage figure applied this way for the run whose exit is being handled is not evaluated against the in-flight token ceiling, because that run has already ended; the run's recorded total carries the figure to the next dispatch decision.
